// scripts.js — improved with animations and better UX
(async function() {
    ...
    function animateOnScroll() {
        const elements = document.querySelectorAll('.card, .hero-inner, .hero-avatar');
    ...
    let projects = [];
    try {
        // Determine correct path regardless of current page depth
        const isInsideAssets = window.location.pathname.includes('/assets/');
        const jsonPath = isInsideAssets ? './projects.json' : 'assets/projects.json';
        const res = await fetch(jsonPath);
        projects = await res.json();

        // 🔧 Fix paths for thumbnails & files dynamically
        const prefix = isInsideAssets ? '' : 'assets/';
        projects.forEach(p => {
            p.thumbnail = prefix + p.thumbnail;
            p.hosted_file = prefix + p.hosted_file;
        });

    } catch (err) {
        console.error('Failed to load projects.json:', err);
        const grid = document.getElementById('project-grid');
        if (grid) grid.innerHTML = '<p style="color:#c00; text-align:center; padding: 2rem;">Could not load projects.json — check that file exists and you are serving the site over HTTP (not file://).</p>';
        return;
    }
    ...
    gridTargets.forEach(grid => {
        ...
        <div class="card-actions">
          ...
          <a class="cta outline" href="${p.hosted_file}" download>Download</a>
        </div>
      `;
            grid.appendChild(el);
        });
    ...
    document.body.addEventListener('click', async(e) => {
        ...
        if (viewBtn) {
            ...
            // footer links
            const driveLink = p.google_drive ? `<a class="cta outline" href="${p.google_drive}" target="_blank" rel="noopener">View in Google Drive</a>` : '';
            const dropboxLink = p.dropbox ? `<a class="cta outline" href="${p.dropbox}" target="_blank" rel="noopener" style="margin-left:8px">View in Dropbox</a>` : '';
      ...
      `;
            ...
            try {
                ...
                previewArea.innerHTML = `<div class="sheet-container">${html}</div>`;
                ...
                if (tbl) {
                    ...
                    // Add some basic styling to table cells
            ...
            try {
                ...
                if (res.ok) {
                    if (msg) {
                        ...
                        msg.style.color = 'var(--excel)';
                    }
                    contactForm.reset();
                } else {
                    if (msg) {
                        msg.textContent = 'There was a problem sending the message.';
                        msg.style.color = '#c00';
                    }
                }
            } catch (err) {
                if (msg) {
                    msg.textContent = 'Network error. Try emailing directly.';
                    msg.style.color = '#c00';
                }
            } finally {
                // Reset button state
